...
export class BlogService extends PrismaClient implements OnModuleInit {
  ...
  async create(createBlogDto: CreateBlogDto, image: Express.Multer.File) {
    ...
    await this.uploadService.upload(image, blog.image!);
    return blog;
  }
  ...
  async findAll() {
    ...
    const blogWithImage = await Promise.all(
      blogs.map(async (blog) => {
        const url = await this.uploadService.getSignedUrl(blog.image!);
  ...
  async remove(id: string) {
    ...
    await this.uploadService.delete(blog.image!);
    return blog;
  }
}
